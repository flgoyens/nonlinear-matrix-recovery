...
% 
%
% Attention: sigma, d,c and PUorth are global variables. 
n = data.n;
s = data.s;
r = data.r;
...
if(isfield(data, 'x0'))
    X = data.x0.X;
     X = proj(X,0.95); 
      X(data.mask) = data.samples;
else
    X = problem.M.rand();
    X = proj(X); % this might not satisfy the measurements 
    X(data.mask) = data.samples;
end
...
%         store = prepare(x, store);
        fx = trace( store.PUorth*kernel_rbf(x,sigma));
        store = incrementcounter(store, 'costcalls');
    end
