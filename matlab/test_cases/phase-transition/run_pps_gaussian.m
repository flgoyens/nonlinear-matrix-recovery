...
RHO = 0.9;
PPS = 10;
recovery = zeros(length(RHO),length(PPS));
distance = zeros(length(RHO),length(PPS));

AMgrad = zeros(length(RHO), length(PPS), ntrials, 7);
AMtr = zeros(length(RHO), length(PPS), ntrials, 7);
RTR = zeros(length(RHO), length(PPS), ntrials, 7);
...
for i = 1: length(RHO)
    ...
    save('results_gaussian_pps_20trials.mat','RTR','AMgrad','AMtr');
